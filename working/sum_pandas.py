...
df = pd.read_excel('input_file_large.xlsx', sheet_name='Sheet1')

end_time_r = time.time()
print(f"読み取り時間: {end_time_r - start_time_r:.6f} 秒")
print(df.head(2))

# ユーザー入力
datefmt = input("dateフォーマットを入力してください（例：%Y-%m-%d、%Y/%m/%dなど：")
subject = input("出力したい工事番号を入力してください（例：国語、数学、英語など）: ")
...
# 工事番号、所属のデータを高速フィルタリング
df_filtered = df.query("所属 == @affiliation and 工事番号 == @subject").copy()
...
df_filtered = df_filtered.assign(
    年度=df_filtered['作業日'].dt.year,
    月=df_filtered['作業日'].dt.month
)
...
columns = ['氏名'] + [f'{i}月' for i in range(4, 13)] + [f'{i}月' for i in range(1, 4)] + ['年間合計', '工事番号', '年度', '所属']

...
【修正履歴】
- assemble_system: Python二重ループをNumPyベクトル演算に置き換え（高速化）
- conductor_charge_per_length: Python二重ループをNumPyベクトル演算に置き換え（高速化）
- model_ratio: ジェネレータ式をタプルに変更（意図を明確化・安全化）
"""
...
def assemble_system(geom, V1, V2, Vshield=0.0):
    ...
    [修正] Python二重ループをNumPyベクトル演算に置き換えた。
    各方向のシフト演算でまとめて隣接ノードを判定し、
    COO形式のデータを一括構築する。
    """
    ...
    all_rows = [row_ids]
    all_cols = [row_ids]
    all_data = [np.full(n_unknown, 4.0)]
    ...
    for di, dj in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
        ...
        in_bounds = (ii >= 0) & (ii < ny) & (jj >= 0) & (jj < nx)

        # 各カテゴリのマスクを一括判定
        is_diel = np.zeros(n_unknown, dtype=bool)
        is_c1   = np.zeros(n_unknown, dtype=bool)
        is_c2   = np.zeros(n_unknown, dtype=bool)

        ib_idx = np.where(in_bounds)[0]
        is_diel[ib_idx] = dielectric[ii[ib_idx], jj[ib_idx]]
        is_c1  [ib_idx] = core1     [ii[ib_idx], jj[ib_idx]]
        is_c2  [ib_idx] = core2     [ii[ib_idx], jj[ib_idx]]

        # 誘電体隣接 → 非対角成分
        diel_idx = np.where(is_diel)[0]
        if len(diel_idx) > 0:
            all_rows.append(row_ids[diel_idx])
            all_cols.append(ids[ii[diel_idx], jj[diel_idx]])
            all_data.append(np.full(len(diel_idx), -1.0))

        # 右辺ベクトル（境界条件）
        b[is_c1] += V1
        b[is_c2] += V2
        # 範囲外 or シールド（core1/core2/dielectric のいずれでもない）→ Vshield
        is_shield = ~is_diel & ~is_c1 & ~is_c2
        b[is_shield] += Vshield

    rows_arr = np.concatenate(all_rows)
    cols_arr = np.concatenate(all_cols)
    data_arr = np.concatenate(all_data)
    A = csr_matrix((data_arr, (rows_arr, cols_arr)), shape=(n_unknown, n_unknown))
    return A, b
...
def conductor_charge_per_length(phi, conductor_mask, dielectric_mask, Vc, eps):
    ...
    [修正] Python二重ループをNumPyのスライスシフト演算に置き換えた。
    各方向について conductor_mask と dielectric_mask の積でアクティブな
    界面辺を一括検出し、まとめて積分する。
    """
    q = 0.0

    # 方向ごとのスライス定義: (conductor側スライス, dielectric側スライス)
    # di=-1: conductor[1:, :] と dielectric[:-1, :] が隣接
    # di=+1: conductor[:-1, :] と dielectric[1:, :]  が隣接
    # dj=-1: conductor[:, 1:] と dielectric[:, :-1] が隣接
    # dj=+1: conductor[:, :-1] と dielectric[:, 1:]  が隣接
    slices = [
        (conductor_mask[1:,  :], dielectric_mask[:-1, :], phi[:-1, :]),
        (conductor_mask[:-1, :], dielectric_mask[1:,  :], phi[1:,  :]),
        (conductor_mask[:,  1:], dielectric_mask[:,  :-1], phi[:,  :-1]),
        (conductor_mask[:, :-1], dielectric_mask[:,  1:],  phi[:,  1:]),
    ]

    for cond_s, diel_s, phi_s in slices:
        active = cond_s & diel_s
        q += eps * (Vc - phi_s[active]).sum()
...
def richardson_extrapolation_three(h1, c1, h2, c2, h3, c3, p_min=0.05, p_max=8.0, nscan=4000):
    ...
    def model_ratio(p):
        # [修正] ジェネレータ式をタプルに変更。
        # 元のコード: return (h**p for h in (h1, h2, h3))
        # ジェネレータは一度消費すると再利用できず、意図が不明確になるため
        # タプルで明示的に値を返す。
        return (h1**p, h2**p, h3**p)
